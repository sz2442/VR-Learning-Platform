...
xAPI statements flow to a Learning Record Store (LRS) like SCORM Cloud or a custom backend, enabling cross-platform learning analytics."
        ...
        var finalMcq = new List<Question>
        {
            ...
            // ── Difficulty 7 ──────────────────────────────────────────────────
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "In the Item Response Theory (IRT) 3-parameter logistic model, which parameter represents the probability of a very low-ability learner answering correctly by chance?", DifficultyLevel = 7, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Discrimination parameter (a)", IsCorrect = false }, new() { Text = "Difficulty parameter (b)", IsCorrect = false }, new() { Text = "Guessing parameter (c)", IsCorrect = true }, new() { Text = "Slip parameter (s)", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "Which phenomenon occurs when a VR learner's virtual hand conflicts with proprioceptive signals from their real hand, reducing sense of embodiment?", DifficultyLevel = 7, Category = "VR Fundamentals", Answers = new List<Answer> { new() { Text = "Vergence-accommodation conflict", IsCorrect = false }, new() { Text = "Visuo-motor mismatch", IsCorrect = true }, new() { Text = "Sensory substitution", IsCorrect = false }, new() { Text = "Perceptual narrowing", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "In multi-armed bandit algorithms applied to adaptive content selection, what does the 'exploration vs exploitation' trade-off refer to?", DifficultyLevel = 7, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Choosing between VR and desktop delivery", IsCorrect = false }, new() { Text = "Balancing trying new content items versus sticking with proven effective ones", IsCorrect = true }, new() { Text = "Deciding between formative and summative assessment", IsCorrect = false }, new() { Text = "Allocating GPU resources between rendering and ML inference", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "The Proximal Policy Optimization (PPO) algorithm is relevant to adaptive learning because it enables:", DifficultyLevel = 7, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Faster rendering of VR environments at high frame rates", IsCorrect = false }, new() { Text = "Reinforcement learning agents to optimize long-term pedagogical sequences", IsCorrect = true }, new() { Text = "Lossless compression of xAPI learning records", IsCorrect = false }, new() { Text = "Real-time collaborative filtering across multiple learner groups", IsCorrect = false } } },

            // ── Difficulty 8 ──────────────────────────────────────────────────
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "Computerized Adaptive Testing (CAT) selects the next item based on maximizing which statistical criterion?", DifficultyLevel = 8, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Kullback-Leibler divergence between prior and posterior ability distributions", IsCorrect = false }, new() { Text = "Fisher Information at the current ability estimate", IsCorrect = true }, new() { Text = "Bayesian posterior mean of the item difficulty parameter", IsCorrect = false }, new() { Text = "Cronbach's alpha of the remaining item bank", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "Which limitation of Deep Knowledge Tracing (DKT) is addressed by the DKVMN (Dynamic Key-Value Memory Networks) model?", DifficultyLevel = 8, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "DKT cannot handle more than 100 knowledge concepts simultaneously", IsCorrect = false }, new() { Text = "DKT treats knowledge as a single hidden state, failing to disentangle individual concept mastery", IsCorrect = true }, new() { Text = "DKT requires GPU acceleration unavailable in browser-based deployments", IsCorrect = false }, new() { Text = "DKT cannot process time-stamped interaction sequences", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "When calibrating a VR eye-tracking system for foveated rendering in an educational context, which metric most directly quantifies the risk of missing critical on-screen content due to gaze prediction error?", DifficultyLevel = 8, Category = "VR Hardware", Answers = new List<Answer> { new() { Text = "Frames per second (FPS) variance", IsCorrect = false }, new() { Text = "Angular accuracy of the gaze prediction in degrees of visual angle", IsCorrect = true }, new() { Text = "Interpupillary distance (IPD) calibration offset", IsCorrect = false }, new() { Text = "Vergence-accommodation conflict threshold in dioptres", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "In a zone of proximal development–aware adaptive system, the learner model update rule most consistent with Vygotskian theory would prioritize:", DifficultyLevel = 8, Category = "Adaptive Learning Theory", Answers = new List<Answer> { new() { Text = "Content at the learner's current mastery ceiling to maximize reinforcement", IsCorrect = false }, new() { Text = "Content just beyond current mastery where scaffolded success is achievable", IsCorrect = true }, new() { Text = "Random content sampling to maximize exploration of the knowledge space", IsCorrect = false }, new() { Text = "Previously failed items until 100% accuracy before progression", IsCorrect = false } } },

            // ── Difficulty 9 ──────────────────────────────────────────────────
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "A VR training system logs fine-grained behavioral telemetry (head pose, gaze, controller kinematics) at 90 Hz. Which evidence-centred design (ECD) layer is responsible for defining which observable actions constitute evidence of a specific competency?", DifficultyLevel = 9, Category = "VR Assessment", Answers = new List<Answer> { new() { Text = "Student model layer", IsCorrect = false }, new() { Text = "Evidence model layer", IsCorrect = true }, new() { Text = "Task model layer", IsCorrect = false }, new() { Text = "Presentation model layer", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "In a federated adaptive learning system where model updates are aggregated across institutions without sharing raw data, which attack exploits gradient updates to reconstruct individual training samples?", DifficultyLevel = 9, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Poisoning attack", IsCorrect = false }, new() { Text = "Gradient inversion attack", IsCorrect = true }, new() { Text = "Model extraction attack", IsCorrect = false }, new() { Text = "Membership inference attack", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "When applying transfer learning to a new educational domain with sparse learner data, which technique specifically addresses catastrophic forgetting while allowing the model to incorporate new knowledge concepts?", DifficultyLevel = 9, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Dropout regularization", IsCorrect = false }, new() { Text = "Elastic Weight Consolidation (EWC)", IsCorrect = true }, new() { Text = "Batch normalization", IsCorrect = false }, new() { Text = "Learning rate warmup scheduling", IsCorrect = false } } },

            // ── Difficulty 10 ─────────────────────────────────────────────────
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "A researcher designs a VR-based adaptive ITS and wants to prove causal efficacy over a passive VR control condition. Which experimental design best controls for demand characteristics, Hawthorne effect, and content confounds simultaneously?", DifficultyLevel = 10, Category = "VR Assessment", Answers = new List<Answer> { new() { Text = "Pre-post quasi-experiment with matched cohorts", IsCorrect = false }, new() { Text = "Double-blind RCT with an active yoked control receiving the same content sequence but without adaptive branching", IsCorrect = true }, new() { Text = "A/B test with random assignment to adaptive vs. linear conditions in production", IsCorrect = false }, new() { Text = "Single-subject reversal (ABA) design with crossover to adaptive condition", IsCorrect = false } } },
            new() { CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "mcq", Text = "An adaptive VR system models learner state as a Partially Observable Markov Decision Process (POMDP). The system cannot directly observe true knowledge state, only noisy quiz responses. Which algorithmic approach is most theoretically grounded for computing the optimal teaching policy under this formulation?", DifficultyLevel = 10, Category = "ML for Learning", Answers = new List<Answer> { new() { Text = "Q-learning with epsilon-greedy exploration over discrete difficulty levels", IsCorrect = false }, new() { Text = "Point-based value iteration (PBVI) over the belief-state space of learner knowledge", IsCorrect = true }, new() { Text = "Supervised imitation learning from expert tutor demonstration sequences", IsCorrect = false }, new() { Text = "Monte Carlo tree search over the curriculum graph with UCB1 selection", IsCorrect = false } } }
        };
        ...
        var finalDragDrop = new List<Question>
        {
            ...
            {
                CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "dragdrop",
                Text = "Match each IRT model parameter to its correct definition",
                DifficultyLevel = 7, Category = "ML for Learning",
                DragDropDataJson = @"{""items"":[{""id"":""i1"",""text"":""Discrimination (a)""},{""id"":""i2"",""text"":""Difficulty (b)""},{""id"":""i3"",""text"":""Guessing (c)""},{""id"":""i4"",""text"":""Slip (1-d)""}],""zones"":[{""id"":""z1"",""label"":""Slope of the item characteristic curve"",""correctItemId"":""i1""},{""id"":""z2"",""label"":""Ability level at 50% correct probability"",""correctItemId"":""i2""},{""id"":""z3"",""label"":""Lower asymptote for low-ability learners"",""correctItemId"":""i3""},{""id"":""z4"",""label"":""Probability of incorrect response despite mastery"",""correctItemId"":""i4""}]}"
            },
            new()
            {
                CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "dragdrop",
                Text = "Match each reinforcement learning component to its adaptive learning equivalent",
                DifficultyLevel = 7, Category = "ML for Learning",
                DragDropDataJson = @"{""items"":[{""id"":""i1"",""text"":""Agent""},{""id"":""i2"",""text"":""State""},{""id"":""i3"",""text"":""Action""},{""id"":""i4"",""text"":""Reward""}],""zones"":[{""id"":""z1"",""label"":""Adaptive tutoring system"",""correctItemId"":""i1""},{""id"":""z2"",""label"":""Current learner knowledge model"",""correctItemId"":""i2""},{""id"":""z3"",""label"":""Next content item or difficulty level selected"",""correctItemId"":""i3""},{""id"":""z4"",""label"":""Learning gain or quiz score improvement"",""correctItemId"":""i4""}]}"
            },
            ...
            {
                CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "dragdrop",
                Text = "Match each ECD (Evidence-Centered Design) layer to its function in VR assessment",
                DifficultyLevel = 8, Category = "VR Assessment",
                DragDropDataJson = @"{""items"":[{""id"":""i1"",""text"":""Competency model""},{""id"":""i2"",""text"":""Evidence model""},{""id"":""i3"",""text"":""Task model""},{""id"":""i4"",""text"":""Presentation model""}],""zones"":[{""id"":""z1"",""label"":""Defines the knowledge and skills to be assessed"",""correctItemId"":""i1""},{""id"":""z2"",""label"":""Maps observable behaviors to latent competencies"",""correctItemId"":""i2""},{""id"":""z3"",""label"":""Specifies the VR scenario context and constraints"",""correctItemId"":""i3""},{""id"":""z4"",""label"":""Controls how the VR environment is rendered to the learner"",""correctItemId"":""i4""}]}"
            },
            ...
            {
                CourseId = vrCourse.Id, QuizType = "finalquiz", QuestionType = "dragdrop",
                Text = "Match each federated learning threat to its correct mitigation strategy",
                DifficultyLevel = 9, Category = "ML for Learning",
                DragDropDataJson = @"{""items"":[{""id"":""i1"",""text"":""Gradient inversion attack""},{""id"":""i2"",""text"":""Model poisoning""},{""id"":""i3"",""text"":""Membership inference""},{""id"":""i4"",""text"":""Free-rider attack""}],""zones"":[{""id"":""z1"",""label"":""Differential privacy noise injection"",""correctItemId"":""i1""},{""id"":""z2"",""label"":""Byzantine-fault-tolerant aggregation (e.g. Krum)"",""correctItemId"":""i2""},{""id"":""z3"",""label"":""Secure multi-party computation"",""correctItemId"":""i3""},{""id"":""z4"",""label"":""Contribution-based incentive mechanism"",""correctItemId"":""i4""}]}"
            }
        };
